...
produce_NA(data, mechanism = "MCAR", perc.missing = 0.5, self.mask=NULL, idx.incomplete = NULL, idx.covariates = NULL, weights.covariates = NULL, by.patterns = FALSE, patterns = NULL, freq.patterns = NULL, weights.patterns = NULL, logit.model = "RIGHT", seed = NULL)
...
writeLines(paste0("Percentage of newly generated missing values (only w.r.t. to incomplete variables): ", sum(R.miss)/prod(dim(R.miss[,1:2]))))
...
```{r idx.covariates}
# Sample missing data for the first two variables in X ---------------------------------------
miss <- produce_NA(X.complete, mechanism="MAR", perc.missing = 0.2, idx.incomplete = c(1), idx.covariates = c(0,1,0,0), weights.covariates = c(0,1,0,0))

X.miss <- miss$data.incomp
R.miss <- miss$idx_newNA

writeLines(paste0("Percentage of newly generated missing values (only w.r.t. to incomplete variables): ", sum(R.miss)/length(R.miss[,1])))
matrixplot(X.miss,  cex.axis = 0.5, interactive = F)
```
...
writeLines(paste0("Percentage of newly generated missing values (only w.r.t. to incomplete variables): ", sum(R.miss)/prod(dim(R.miss))))
matrixplot(X.miss,  cex.axis = 0.5, interactive = F)
```
...
miss <- produce_NA(X.complete, mechanism="MAR", perc.missing = 0.2, idx.incomplete = c(1,4), by.patterns = T, patterns = matrix(c(0,1,1,1,                                                                                                                  1,1,1,0), ncol = 4, byrow=T))

X.miss <- miss$data.incomp
R.miss <- miss$idx_newNA

writeLines(paste0("Percentage of newly generated missing values (only w.r.t. to incomplete variables): ", sum(R.miss)/(dim(R.miss)[1]*2)))
matrixplot(X.miss,  cex.axis = 0.5, interactive = F)
```
...
```{r patterns_3}
# Sample missing data by using the by.patterns option and user-specified patterns ----
miss <- produce_NA(X.complete, mechanism="MAR", perc.missing = 0.2, idx.incomplete = c(1,3,4), by.patterns = T, patterns = matrix(c(0,1,1,1,                                                                                                                  1,1,0,0), ncol = 4, byrow=T), freq.patterns = c(0.2, 0.8))

X.miss <- miss$data.incomp
R.miss <- miss$idx_newNA

writeLines(paste0("Percentage of newly generated missing values (only w.r.t. to incomplete variables): ", sum(R.miss)/(dim(R.miss)[1]*3)))
matrixplot(X.miss,  cex.axis = 0.5, interactive = F)
```
